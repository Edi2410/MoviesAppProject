...
public class SqlRepository implements IRepository {
    ...
    @Override
    public List<People> getMovieActors(int ID) throws Exception {
    ...
    @Override
    public List<People> getMovieDirectors(int ID) throws Exception {
        List<People> directors = new ArrayList<>();
        DataSource dataSource = DataSourceSingleton.getInstance();
        try (Connection con = dataSource.getConnection(); CallableStatement stmt = con.prepareCall(SELECT_MOVIE_DIRECTORS);) {
            stmt.setInt(IDMOVIE, ID);
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    directors.add(new People(
                            rs.getInt(ID_USER_FOR_MOVIES),
                            rs.getString(PEOPLE_NAMES)
                    ));
                }
